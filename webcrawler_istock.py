...
start = time.time()
if ip_ask == 'no':
    web_path = f"https://www.istockphoto.com/search/2/image?phrase={input_category}&mediatype=photography&page="
else:
    ip = str(ip_ask)
    web_path = f'https://www.istockphoto.com/search/more-like-this/{ip}?phrase={input_category}&mediatype=photography&page='
    
browser = webdriver.Chrome(ChromeDriverManager().install())
path = f"istock_images/{input_category}"
current_path = os.path.abspath(os.getcwd())
print("Downloading to",current_path + '\ISTOCK_images\\' + str(input_category) )
...
for page in range(1,int(page_num)+1):# 執行1~2頁
    ...
    results = soup.find_all("img",{"class":"MosaicAsset-module__thumb___klD9E"})
